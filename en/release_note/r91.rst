...
This document includes information on CUBRID 9.1 (Build No. 9.1.0.0212). 
...
Refer to previously specified columns in the VALUES and SET clauses of INSERT statement(CUBRIDSUS-3112)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

It is allowed to refer to the previously specified columns in the VALUES and SET clauses of INSERT statement. For example, when the expression like "b = a + 1" is evaluated, a column's value which was previously specified can be referred.
...
In the above example, the previous version cannot evaluate b's value, but the fixed version can evaluate b's value as 1 by referring to a's value.
...
The query is immediately stopped when it just reaches the result count of LIMIT clause. It does not go further to find the N+1 record anymore.
...
Fix the malfunction when the subquery which refers to the source table on MERGE statement is used as target table.
...
Continuous failure of requests from the application when the number of CASes was smaller than the load on the shard proxy(CUBRIDSUS-10012)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
