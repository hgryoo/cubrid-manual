...
The collation and charset of an ENUM type column is no longer propagated from its components(CUBRIDSUS-12269)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
In previous version, the column 'e' had charset UTF8, collation utf8_bin (propagated from string literal components).

In current version, the column 'e' has charset iso88591, collation iso88591_bin. The string literal components are coerced from UTF8 charset to iso88591, when the column is created. If user wants to apply another charset (or collation), this has to be explicitly specified for column or for table: 
