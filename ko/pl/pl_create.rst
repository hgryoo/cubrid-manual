...
CUBRID의 문자열 데이터는 코드셋을 지원한다. 문자열 데이터에 대한 코드셋 처리 방식은 :doc:`/sql/i18n`\를 참고한다.
저장 프로시저를 작성할 때 문자열 데이터의 코드셋은 다음의 규칙으로 지정된다.

* 저장 프로시저와 저장 함수의 문자열 인수의 코드셋은 데이터베이스의 코드셋과 동일하다.
* 저장 함수의 반환 타입이 문자열인 경우, 문자열의 코드셋은 데이터베이스의 코드셋과 동일하다.
* 저장 프로시저와 저장 함수에서 호출하는 질의문으로부터 반환되는 문자열의 코드셋은 질의문의 코드셋과 동일하다.

위의 규칙에 따라 코드셋을 지정한 문자열은 저장 프로시저와 저장 함수 내부적으로 하나의 코드셋 (UTF-16)으로 변환되어 처리된다.
따라서 저장 프로시저 내에서 서로 다른 코드셋을 가진 문자열 간의 연산은 가능하지만, 인수나 반환 값은 데이터베이스의 코드셋으로 변환되므로 주의하여 사용해야 한다.
